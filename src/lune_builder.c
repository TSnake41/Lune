...
typedef struct lune_builder {
  mz_zip_archive zip;
  FILE *file;
} lune_builder;

static int lune_builder_new(lua_State *L)
{
  FILE *self = lua_touserdata(L, -2);
  const char *path = luaL_checkstring(L, -1);

  lune_builder *builder = malloc(sizeof(lune_builder));
...
static int lune_builder_set_executable(lua_State *L)
{
  const char *path = luaL_checkstring(L, -1);
...
static int lune_builder_close(lua_State *L)
{
  lune_builder *builder = lua_touserdata(L, -1);
...
static int lune_builder_add(lua_State *L)
{
  lune_builder *builder = lua_touserdata(L, -3);
...
int lune_builder_boot(lua_State *L, FILE *self)
{
  ...
  lua_pushcfunction(L, lune_builder_new);
  lua_setglobal(L, "builder_new");

  lua_pushcfunction(L, lune_builder_close);
  lua_setglobal(L, "builder_close");

  lua_pushcfunction(L, lune_builder_add);
  lua_setglobal(L, "builder_add");

  lua_pushcfunction(L, lune_builder_set_executable);
  lua_setglobal(L, "set_executable");

  if (luaL_dostring(L, lune_builder_lua))
